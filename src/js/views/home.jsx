...
export const Home = () => {
	...
	return(
		...
		<div className="d-flex scrollable">
		{store.planets.map((item,index)=><Card2 item={item} key={item} name={item.name} popu={item.population} terra={item.terrain} id={index+1}/>)}
		</div>
		<h1 className="text-danger my-4 ">Vehicles</h1>
		<div className="d-flex scrollable">
		{store.vehicles.map((item,index)=><Card3 item={item} key={item} name={item.name} model={item.model} passengers={item.passengers} id={index+1}/>)}
		</div>
	</>
);
}
