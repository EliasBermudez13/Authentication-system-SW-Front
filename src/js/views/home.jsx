...
export const Home = () => {
	...
	return(
		...
		<div className="d-flex scrollable">
			{allPeople.map((item,index)=><Card key={item} name={item.name} gender={item.mass} hair={item.hair_color} 
			 eye={item.eye_color} id={index+1}/>)}
		</div>
		</div>
		<h1 className="text-danger my-4">Planets</h1>
		<div className="d-flex scrollable">
		{allPlanets.map((item,index)=><Card2 key={item} name={item.name} popu={item.population} terra={item.terrain} id={index+1}/>)}
		</div>
		<h1 className="text-danger my-4 ">Vehicles</h1>
		<div className="d-flex scrollable">
		{allVehicles.map((item,index)=><Card3 key={item} name={item.name} model={item.model} passengers={item.passengers} id={index+1}/>)}
		</div>
	</>
);
}
